# Local Variables
locals {
  resource_group_tags = {
    Project = var.project_name
  }
  resource_tags = {
    Environment = var.environment_name
    CreatedBy   = "Terraform"
  }
}

# Current Connection Config
data "azurerm_client_config" "current" {
}

# Provision a Resource Group
resource "azurerm_resource_group" "rg" {
  name     = "${var.project_prefix}-${var.project_location_prefix}-${var.environment_prefix}-rg"
  location = var.project_location
  tags     = merge(local.resource_group_tags, local.resource_tags)
}

# KeyVault Module
module "key_vault" {
  source                = "../modules/key_vault"
  kv_project_prefix     = var.project_prefix
  kv_project_location   = var.project_location_prefix
  kv_environment_prefix = var.environment_prefix
  kv_location           = var.project_location
  kv_resource_group     = azurerm_resource_group.rg.name
  kv_tenant_id          = data.azurerm_client_config.current.tenant_id
  kv_object_id          = data.azurerm_client_config.current.object_id
  kv_tags               = local.resource_tags
}

# Web App Module
module "web_app" {
  source                  = "../modules/web_app"
  app_project_prefix      = var.project_prefix
  app_project_location    = var.project_location_prefix
  app_environment_prefix  = var.environment_prefix
  app_location            = var.project_location
  app_resource_group      = azurerm_resource_group.rg.name
  app_instrumentation_key = module.application_insights.instrumentation_key
  app_plan_id             = module.service_plan.plan_id
  app_sql_server_name     = module.sql_server.server_name
  app_sql_database_name   = module.sql_database.sql_database_name
  app_kv_id               = module.key_vault.kv_id
  app_tags                = local.resource_tags
}

# Service Plan Module
module "service_plan" {
  source                  = "../modules/service_plan"
  plan_project_prefix     = var.project_prefix
  plan_project_location   = var.project_location_prefix
  plan_environment_prefix = var.environment_prefix
  plan_location           = var.project_location
  plan_resource_group     = azurerm_resource_group.rg.name
  plan_os_name            = "windows"
  plan_sku_name           = var.plan_sku_name
  plan_worker_count       = var.plan_worker_count
  plan_tags               = local.resource_tags
}

# Application Insight Module
module "application_insights" {
  source                 = "../modules/application_insights"
  ais_project_prefix     = var.project_prefix
  ais_project_location   = var.project_location_prefix
  ais_environment_prefix = var.environment_prefix
  ais_location           = var.project_location
  ais_resource_group     = azurerm_resource_group.rg.name
  ais_tags               = local.resource_tags
}

# SQL Server Module
module "sql_server" {
  source                 = "../modules/sql_server"
  sql_project_prefix     = var.project_prefix
  sql_project_location   = var.project_location_prefix
  sql_environment_prefix = var.environment_name
  sql_location           = var.project_location
  sql_resource_group     = azurerm_resource_group.rg.name
  sql_kv_id              = module.key_vault.kv_id
  sql_tags               = local.resource_tags
}

# SQL Database Module
module "sql_database" {
  source                = "../modules/sql_database"
  db_project_prefix     = var.project_prefix
  db_project_location   = var.project_location_prefix
  db_environment_prefix = var.environment_prefix
  db_location           = var.project_location
  db_resource_group     = azurerm_resource_group.rg.name
  db_sql_server_id      = module.sql_server.server_id
  db_sql_sku            = var.db_sql_sku
  db_sql_max_size       = var.db_sql_max_size
  db_tags               = local.resource_tags
}